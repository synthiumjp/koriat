...
**Date:** 15 April 2026
**Status:** v1.1 — locked, ready for OSF posting prior to main collection. v1.1 supersedes v1.0 after sanity-run diagnostic findings; see §10 for full change log.
...
- **E-base (implicit base-vs-instruct contrast, Llama-3-8B):** Compare the distributions of `length_normalised_logprob` between M1 (Llama-3-8B base) and M2 (Llama-3-8B-Instruct) on the NUM condition, restricted to trials in which both models produced non-empty output (`response_length_tokens > 0`). Two-sample Kolmogorov-Smirnov test on the raw length-normalised-logprob values. Pre-registered direction: the instruct model's logprob distribution will be more compressed (higher mean, lower entropy) than the base model's, consistent with post-training-induced narrowing of the answer-token probability distribution. Report the KS statistic, p-value, mean per group, entropy per group, and empirical CDFs of both distributions. **This is a behavioural analogue to the post-training readout-compression hypothesis at the *implicit* measurement level**, and is explicitly not the same test as v1.0's dropped H3, which would have operated on verbalised confidence. The implicit measurement is viable for M1 because M1 produces answer tokens with logprobs natively, regardless of whether the continuation prompt elicits a verbal confidence rating (it does not). E-base is consistent with — but not diagnostic of — the readout-layer mechanism proposed in Cacioli (2026, PCN paper) and the circuit-level evidence in Zhao et al. (2026). Note that length-normalised logprob is not the same quantity as verbal confidence; the Miao & Ungar (2026) orthogonality finding implies these signals may diverge. E-base tests the implicit signal only and is not used for confirmatory inference.
...
1. Pre-registration drafts (v0.1, v0.2, v0.3, v0.4, v1.0, v1.1) circulated for external review by four LLM reviewers across two rounds, plus pre-collection verification trials on M8 (§3.7), pre-posting parser-input clarification on simulated data (now folded into §3.5), and post-sanity diagnostic revisions (§10 v1.0 → v1.1). Reviews and verification findings integrated.
...
4. Pre-registration v1.1 finalised and posted to OSF as a new project.
...
- This pre-registration document (v1.1)
...
**Quantisation:** All models run at Q5_K_M. Cacioli (2026, arXiv:2604.08976) showed that quantisation can affect metacognitive measurement at the M-ratio level, while AUROC₂ is robust to this manipulation. The present study uses AUROC₂ as the primary Type-2 metric for this reason. Nonetheless, full-precision behaviour is not directly tested here.
...
This pre-registration evolved across multiple drafts before reaching v1.1. v1.1 is the version posted to OSF; earlier drafts (v0.1 through v1.0) are documented here for transparency but are not themselves OSF artifacts.
...
v1.1 is posted to OSF. The sanity run outputs from the v1.0 script (which are the outputs that surfaced these issues) are committed to OSF alongside v1.1 as `sanity_run_v10_pre_fix.parquet` so that readers can verify the diagnostic provenance of the v1.0 → v1.1 revisions. A fresh sanity run under the v1.1 script is committed separately as `sanity_run_v11.parquet`.

---

## 11. Summary of locked v1.1 parameters

For audit convenience, the locked v1.1 design is summarised here:
...
- **Confirmatory hypotheses (4):** H1 (saturation prevalence > 60% on 7 instruct, NUM), H2 (≥4 of 7 Invalid on NUM), H4 (≥2 NUM-Invalid models reclassified non-Invalid under CAT), H5 (mean R²_CV < 0.20 both conditions). H3 was a confirmatory hypothesis in v1.0 and is retired in v1.1; see §2 and §10.
